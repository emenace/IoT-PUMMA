...
module.exports = {
    ...
    async get_100Data(req, res) {
        ...
        console.log("[REST-API Canti] GET 100 Data");
    },
    ...
    async get_countData(req, res) {
        ...
        console.log(`[REST-API Canti] GET ${count} Data`);
    ...
    async get_pagination(req, res) {
        ...
        countData = await dbase_rest.query(`SELECT count(*) as total FROM mqtt_canti`);
        ...
        console.log(`[REST-API Canti] GET ALL DATA BY PAGE. PAGE ${page} OF ${totalPage}`);
    },
    ...
    async get_paginationCount(req, res) {
        ...
        console.log(`[REST-API Canti] GET DATA BY PAGE. ${req.params.count} DATA WITH LIMIT ${req.query.limit} OFFSET ${req.query.offset} `);
    },
    ...
    async get_byTime_obj(req, res) {
        ...
        if (timer == "second" || timer == "minute" || timer == "hour" || timer == "day") {
            ...
            FROM mqtt_canti WHERE datetime >= now() - Interval '${time}' ${req.query.timer} ORDER BY datetime DESC`, function (err, result) {
                if (err) {
                ...
                console.log(`[REST-API Canti] GET ${dataColumn} FOR ${time} ${timer} AS OBJECT`);
            });
        } else {
            res.status(404);
            res.json({
    ...
    async get_byTime_list(req, res) {
        ...
        if (timer == "second" || timer == "minute" || timer == "hour" || timer == "day") {
            dbase_rest.query(`SELECT datetime, ${dataColumn} as data
                ...
                console.log(`[REST-API Canti] GET ${dataColumn} FOR ${time} ${timer} AS LIST`);
            });
        } else {
            res.status(404);
            res.json({
    ...
    async get_byDate_obj(req, res) {
        ...
        dbase_rest.query(`SELECT datetime as utc, ${dataColumn} as data
            FROM mqtt_canti_stored WHERE datetime BETWEEN SYMMETRIC '${dateStart}' AND '${dateEnd} 23:59:59' ORDER BY datetime DESC`, function (err, result) {
            if (err) {
                console.log(err.message)
                res.status(404);
            ...
            console.log(`[REST-API Canti] GET ${dataColumn} DATA FROM ${dateStart} TO ${dateEnd} AS OBJECT`);
        });
        // });
    },
    ...
    async get_byTime_list_all(req, res) {
        ...
        if (timer == "second" || timer == "minute" || timer == "hour" || timer == "day") {
            dbase_rest.query(`SELECT datetime, waterlevel, voltage, temperature, forecast30, forecast300, rms, threshold, alertlevel
                ...
                console.log(`[REST-API Canti] GET ALL DATA FOR ${time} ${timer} AS LIST`);
            });
        } else {
            res.status(404);
            res.json({
    ...
    async get_all_interval(req, res) {
        ...
        // select datetime, waterlevel, voltage, temperature, forecast30, forecast300, rms, threshold, alertlevel from mqtt_canti
        // where datetime in(
        ...
        var interval;//seconds (5 minute)
        let regex_number = time.match(/(\d+)/);
        let regex_text = time.match(/\b\w{3,}\b/g);
        ...
        dbase_rest.query(`             
            ...
            console.log(`[REST-API Canti] GET ALL DATA WITH TIME ${time} AND INTERVAL 1 Minute AS LIST`);
        });
    },
    ...
    async get_all_interval_date(req, res) {
        ...
        // select datetime, waterlevel, voltage, temperature, forecast30, forecast300, rms, threshold, alertlevel from mqtt_canti_stored
        // where datetime in(
        //     select generate_series(first_date, last_date, '1 minute'::interval)::timestamp as date_hour
        //     from dateRange
        // )
        ...
        dbase_rest.query(`
            ...
            console.log(`[REST-API Canti] GET ALL DATA BETWEEN ${starts} AND ${end}`);
        });
    },
}